import {
    ...
    Space, Bone, KeyboardInfo, KeyboardEventTypes, Epsilon, Quaternion, Scalar, MeshBuilder, StandardMaterial, Color3, TransformNode,
} from "@babylonjs/core";
...
export default class Player extends Mesh {
    ...
    public onStart(): void {
        ...
        // // Configure ellipsoid
        this.ellipsoid.set(1, 1, 1);
        this.ellipsoidOffset.set(0, 1, 0);

        console.log(`ellipsoid ,  ${this.ellipsoid._x}, ${this.ellipsoid._y}, ${this.ellipsoid._z}`)
        console.log(`ellipsoid offset -  ${this.ellipsoidOffset._x}, ${this.ellipsoidOffset._y}, ${this.ellipsoidOffset._z}`)
        // Configure actions.
        this._actions = {
            "idle": {
                name: "idle",
        ...
        var ellipsoid = MeshBuilder.CreateCylinder("debug", {diameter: (player.ellipsoid.x *2), height: (player.ellipsoid.y * 2), subdivisions: 24}, this._scene);
        ellipsoid.position.copyFrom(player.position);
        ellipsoid.position.addInPlace(player.ellipsoidOffset);
        ...
        //         var ellipsoid = MeshBuilder.CreateCylinder("debug", {diameter: (m.ellipsoid.x *2), height: (m.ellipsoid.y * 2), subdivisions: 24}, this._scene);
        //         ellipsoid.position.copyFrom(m.position);
        //         ellipsoid.position.addInPlace(m.ellipsoidOffset);
    ...
    public onUpdate(): void {
        ...
        console.log('gravity : jump value : ', this._scene.gravity.y, this._jumpValue)
        ...
        const targetWorldPosition = this.position.add(new Vector3(0, 1, 0));

        console.log('Target World Position : ', targetWorldPosition, this.position, this._targetBone.position, boneWorldPosition)
        this._camera.target.copyFrom(targetWorldPosition);
        // this._camera.lockedTarget = this._camRoot;
        // const centerPlayer = this.position.y + .8;
        // this._camRoot.position = Vector3.Lerp(this._camRoot.position, new Vector3(this.position.x, centerPlayer, this.position.z), 0.2)
        // this._camRoot.position = new Vector3(this.position.x, centerPlayer, this.position.z)
